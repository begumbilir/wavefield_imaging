...
% x_D_rec_start = -0.5*lambda; % x-coordinate of the start point of receiver domain
% x_D_rec_end = 1.5*lambda; % x-coordinate of the end point of receiver domain
...
% 
% % Define the reciever domain 2
% x_D_rec_start2 = -0.5*lambda; % x-coordinate of the start point of receiver domain
% x_D_rec_end2 = 1.5*lambda; % x-coordinate of the end point of receiver domain
% y_D_rec2 = -0.5*lambda; % y-coordinate of the receiver domain (does not change)
% % Draw the D^{rec2} in Figure 1
% %line([x_D_rec_start2, x_D_rec_end2], [y_D_rec2, y_D_rec2], 'Color', [0 0 0], 'LineWidth', 3);
% scatter(x_D_rec_start2:h:x_D_rec_end2 , y_D_rec2);
% text(0.5*lambda, y_D_rec2-lambda/4, 'Receiver Domain D^{rec2}', 'HorizontalAlignment', 'center');
% 
% % Define the reciever domain 3
% y_D_rec_start3 = -0.5*lambda+h; % x-coordinate of the start point of receiver domain
% y_D_rec_end3 = 1.5*lambda-h; % x-coordinate of the end point of receiver domain
% x_D_rec3 = -0.5*lambda; % y-coordinate of the receiver domain (does not change)
% 
% % Draw the D^{rec3} in Figure 1
% %line([x_D_rec3, x_D_rec3], [y_D_rec_start3, y_D_rec_end3], 'Color', [0 0 0], 'LineWidth', 3);
% scatter(x_D_rec3, y_D_rec_start3:h: y_D_rec_end3);
% text(x_D_rec3-lambda/4, 0.5*lambda, 'Receiver Domain D^{rec3}', 'HorizontalAlignment', 'right');
% 
% % Define the reciever domain 4
% y_D_rec_start4 = -0.5*lambda+h; % x-coordinate of the start point of receiver domain
% y_D_rec_end4 = 1.5*lambda-h; % x-coordinate of the end point of receiver domain
% x_D_rec4 = 1.5*lambda; % y-coordinate of the receiver domain (does not change)
% 
% % Draw the D^{rec4} in Figure 1
% %line([x_D_rec4, x_D_rec4], [y_D_rec_start4, y_D_rec_end4], 'Color', [0 0 0], 'LineWidth', 3);
% scatter(x_D_rec4, y_D_rec_start4:h: y_D_rec_end4);
% text(x_D_rec4+ lambda/4, 0.5*lambda, 'Receiver Domain D^{rec4}', 'HorizontalAlignment', 'left');
% 
% % Adjust axis
...
%% STEP 10 - Circular Sensor Setup
figure; hold on; axis equal;
xlabel('x'); ylabel('y');
title('Figure 1: Configuration Sketch with Circular Receiver Domain');
...
% Define Circular Receiver Domain
R = 1.5 * lambda;           % Radius of the circle
N_sensors = 40;             % Total number of sensors
theta = linspace(0, 2*pi, N_sensors + 1); 
theta(end) = [];            % Remove the duplicate at 2*pi

% Center coordinates
x_center = lambda / 2;
y_center = lambda / 2;

% Sensor coordinates on the circle
x_circ = x_center + R * cos(theta);
y_circ = y_center + R * sin(theta);

% Plot the sensors
scatter(x_circ, y_circ, 'filled');
text(0, R + lambda/1.5, 'Circular Receiver Domain D^{rec}', ...
     'HorizontalAlignment', 'center');

% Optionally connect them with a circular line for visualization
...
% Adjust axis
% ylim([-R - lambda/2, R + lambda]);
% xlim([-R - lambda/2, R + lambda]);
ylim([-sy/10, sy + 2*lambda]);
set(gca, 'YDir', 'reverse');  % Invert y-axis if required
grid on;
...
x_D_rec_grids2 = x_D_rec_start2 : 3*h : x_D_rec_end2;
y_D_rec_grids3 = y_D_rec_start3 : 3*h : y_D_rec_end3;
y_D_rec_grids4 = y_D_rec_start4 : 3*h : y_D_rec_end4;

M = length(x_D_rec_grids)*2 + length(y_D_rec_grids3)*2; %place measurements across
...
th = length(x_D_rec_grids);
th2 = length(y_D_rec_grids3);
for m = 1:M  
    ...
    elseif m > th && m <= 2*th
        row_Green = sqrt((x_D_rec_grids2(m-th) - X_midpoint).^2 + (y_D_rec2 - Y_midpoint).^2);
    elseif m > 2*th && m <= 2*th + th2
        row_Green = sqrt((x_D_rec3 - X_midpoint).^2 + (y_D_rec_grids3(m-2*th) - Y_midpoint).^2);
    else
        row_Green = sqrt((x_D_rec4 - X_midpoint).^2 + (y_D_rec_grids4(m-(2*th + th2)) - Y_midpoint).^2); 
    end
...
plot(1:length(sing_vals), sing_vals); 
